# main_vacuum.py
"""
Vacuum Scalper Entry Point.

Usage:
    # Shadow mode (default) - logs signals, no execution
    python main_vacuum.py --config config_vacuum.yaml
    
    # Paper mode - small size real execution
    python main_vacuum.py --config config_vacuum.yaml --mode paper
    
    # Live mode - full execution (use with caution!)
    python main_vacuum.py --config config_vacuum.yaml --mode live
"""
import asyncio
import argparse
import signal
...
from hyperbot.vacuum_orchestrator import VacuumOrchestrator, ExecutionMode
from hyperbot.logger import setup_logging, get_logger


def parse_args():
    parser = argparse.ArgumentParser(description="Vacuum Scalper Bot")
    parser.add_argument(
        "--config", "-c",
        type=str,
        default="config_vacuum.yaml",
...
def get_mode(args, config) -> ExecutionMode:
    ...
    if args.mode:
        ...
        mode_str = config.as_dict().get("vacuum", {}).get("mode", "shadow").lower()
...
async def main():
    ...
    if args.debug:
        ...
        log_level = config.as_dict().get("logging", {}).get("level", "INFO").upper()
        
    setup_logging(level=log_level)
    log = get_logger(__name__)
    
    log.info("=" * 50)
    log.info("Vacuum Scalper Starting")
    log.info("Liquidity Vacuum + Micro Break & Reclaim Strategy")
    log.info("=" * 50)
    ...
    log.info("Vacuum Scalper stopped")
